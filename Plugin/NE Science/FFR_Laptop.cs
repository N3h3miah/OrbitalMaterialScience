...
{
    class FFR_Laptop : InternalModule
    {
        private double lastUpdate = 0;
        private const string SCREEN_PART_NAME = "LaptopScreen";

        [KSPField(isPersistant = false)]
        public string folder = "NehemiahInc/Props/FFR_Laptop";
        ...
        public override void OnUpdate()
        {
            ...
            if (lastUpdate + refreshInterval < Time.time)
            {
                ...
                if (lab.hasEquipmentInstalled(EquipmentRacks.FFR))
                {
                    if (running != lab.isEquipmentRunning(EquipmentRacks.FFR))
                    {
                        running = lab.isEquipmentRunning(EquipmentRacks.FFR);
